...
Q_heart = Q_left_heart + Q_right_heart;
Q_lu = Q_pas + Q_pat + Q_par + Q_pcp;


for i = 1:length(t)
GG_l = GG_l0 * ((bloodVolume(i)/5500)*-3.333333+4.33333); %Scale glucose consumption 
[C_h(i,:),Q_h(i)] = organConc(C_a(i,:),bloodVolume(i),perCO_h,OC_h,RQ_h,0,@heartDynamic, Q_heart);
[C_l(i,:),Q_l(i)] = organConc(C_a(i,:),bloodVolume(i),perCO_l,OC_l,RQ_l,GG_l,@liverDyanmic, Q_liver);
[C_b(i,:),Q_b(i)] = organConc(C_a(i,:),bloodVolume(i),perCO_b,OC_b,RQ_b,0,@brainDynamic, Q_brain);
[C_k(i,:),Q_k(i)] = organConc(C_a(i,:),bloodVolume(i),perCO_k,OC_k,RQ_k,GG_k,@kidneyDynamic, Q_kidney);
[C_lung(i,:),Q_lung(i)] = organConc(C_a(i,:),bloodVolume(i),perCO_lung,OC_lung,RQ_lung,0,@lungDynamic, Q_lu);


C_body = [0.035042 0.7109339 0];

%Calculate venous concentrations
C_ven(i,:) = (C_h(i,:).*Q_h(i) + C_l(i,:).*Q_l(i) + C_b(i,:).*Q_b(i) + C_k(i,:).*Q_k(i) + C_lung(i,:).*Q_lung(i))/(mean(Q_heart)*0.67);
C_ven(i,:) = (C_ven(i,:) + C_body)/2;
%disp(C_h(i,:).*Q_h(i) + C_l(i,:).*Q_l(i) + C_b(i,:).*Q_b(i) + C_k(i,:).*Q_k(i) + C_lung(i,:).*Q_lung(i))
%Pulmonary circulation
[C_pul(i,:),Q_pul(i)] = pulmonaryCirculation(C_ven(i,:),bloodVolume(i), Q_heart);
...
function [C_v,Q] = organConc(C_a,volume,perc_CO,OC,RQ,GG,dynamicFun, Qsim)
...
function [C_v,Q] = pulmonaryCirculation(C_a,volume, Qsim)

Q =  mean(Qsim) *cardiacOutputFraction(volume);
