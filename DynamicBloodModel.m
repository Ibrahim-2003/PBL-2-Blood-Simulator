...
[C_h(i,:),Q_h(i)] = organConc(C_a(i,:),bloodVolume(i),perCO_h,OC_h,RQ_h,@heartDynamic);
[C_l(i,:),Q_l(i)] = organConc(C_a(i,:),bloodVolume(i),perCO_l,OC_l,RQ_l,@liverDyanmic);
[C_b(i,:),Q_b(i)] = organConc(C_a(i,:),bloodVolume(i),perCO_b,OC_b,RQ_b,@brainDynamic);
[C_k(i,:),Q_k(i)] = organConc(C_a(i,:),bloodVolume(i),perCO_k,OC_k,RQ_k,@heartDynamic);
[C_lung(i,:),Q_lung(i)] = organConc(C_a(i,:),bloodVolume(i),perCO_lung,OC_lung,RQ_lung,@lungDynamic);

C_body = [0.035042 0.7109339 0];

%Calculate venous concentrations
C_ven(i,:) = (C_h(i,:).*Q_h(i) + C_l(i,:).*Q_l(i) + C_b(i,:).*Q_b(i) + C_k(i,:).*Q_k(i) + C_lung(i,:).*Q_lung(i))/(44.2660*0.72);
disp(C_ven(i,:))
...
figure;
plot(t,C_a(1:(end-1),1));
figure;
plot(t,C_a(1:(end-1),2));
...
function [C_v,Q] = organConc(C_a,volume,perc_CO,OC,RQ,dynamicFun)
...
VG = VCO2/6; %stoichiometric ratio for glycolysis 
